...
export const weekendPlanInputSchema = z.object({
  ...
  planType: z.string().default('TWO_DAY'),
  dayCount: z.number().int().min(1).max(8).default(2),
...
export interface WeekendDayOutput {
  ...
  date: string;
  theme: string;
  weather: { condition: string; tempMin: number; tempMax: number; rainChance: number };
  stops: StopOutput[];
  dayTotalCost: number;
...
export interface StopOutput {
  ...
  totalCost: number;
  currency: string;
  transportFromPrev: { mode: string; distanceMeters: number; durationMinutes: number; estimatedCost: number };
  isHiddenGem: boolean;
  isPhotoSpot: boolean;
  isIndoor: boolean;
  crowdLevel: string;
  ...
  transportOptions?: Array<{ mode: string; emoji: string; durationMin: number; costEstimate: number; available: boolean }>;
}
...
export interface BudgetBreakdownOutput {
  ...
  hotel: CostLineItem & { suggestions: { name: string; price: number; rating: number }[]; hotelOptions?: { name:string; price:number; rating:number; description:string; amenities:string[] }[] };
...
export const aiStopSchema = z.object({
  ...
  category: z.enum(['BREAKFAST','BRUNCH','LUNCH','CAFE_STOP','DINNER','SUPPER','TOURIST_ATTRACTION','HIDDEN_GEM','PHOTO_SPOT','NIGHT_ACTIVITY','SHOPPING','NATURE']),
  emoji: z.string(),
...
export const aiWeekendPlanSchema = z.object({
  ...
  budgetBreakdown: z.object({
    hotel: z.object({ estimatedCost: z.number(), suggestion: z.string(), suggestionRating: z.number(), hotelOptions: z.array(z.object({name:z.string(),price:z.number(),rating:z.number(),description:z.string(),amenities:z.array(z.string())})).optional() }),
